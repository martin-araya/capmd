...
- `configure_logging()` es idempotente: usarla en cada arranque del CLI.
...
def get_logger(name: str | None = None) -> logging.Logger:
    ...
    Acepta tanto `get_logger("cli")` como `get_logger("capmd.cli")` — el
    prefijo `capmd.` se aplica una sola vez.
    """
    if name is None:
